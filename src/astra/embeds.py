...
def get_card_image_path(
    card: Card, reversed: bool = False, user_id: Optional[str] = None
) -> Optional[str]:
    """
    Get the path to a card image.

    If user_id is provided, uses the user's selected theme.
    Falls back to searching common paths if theme doesn't have the card.
    """
    # If user_id provided, try their selected theme first
    if user_id:
        ...
        if theme:
            ...
            # Fall back to default theme if different
            if theme.id != theme_manager.DEFAULT_THEME_ID:
                default_theme = theme_manager.get_theme(theme_manager.DEFAULT_THEME_ID)
                if default_theme:
                    path = default_theme.get_card_path(
                        suit=card.suit.name,
                        number=card.number,
                        reversed=reversed,
                        card_name=card.name,
    ...
    # Legacy fallback: search common paths directly
...
def create_reading_embed_with_images(
    ...
    Args:
        reading: The reading result
        user: Discord user who requested the reading
        spread: The spread configuration
        user_id: User ID as string
        theme_id: Optional theme ID. If not provided, will be auto-detected.
    ...
    for i, result in enumerate(reading.results):
        ...
        # Get card image using user's selected theme
        image_path = get_card_image_path(result.card, result.reversed, user_id)
    ...
    for i, result in enumerate(reading.results):
        ...
        emoji = (
            POSITION_EMOJIS[result.position - 1]
            if result.position <= len(POSITION_EMOJIS)
            else "🃏"
        )
        ...
        if position:
            position_title = f"{emoji} **{position.name}**"
        else:
            position_title = f"{emoji} Card {result.position}"
    ...
    # Footer with RWS attribution for classic theme
    footer_text = f"Reading for {user.display_name} • {spread.num_cards} cards"

    # Auto-detect theme for attribution if not provided
    if theme_id is None:
        try:
            from .themes import get_theme_manager
            manager = get_theme_manager()
            theme = manager.get_user_theme(user_id) if manager else None
            theme_id = theme.id if theme else None
        except Exception:
            theme_id = None

    if theme_id in RWS_THEME_IDS:
        footer_text += "\n🎨 Artwork by Pamela Colman Smith (1909)"

    embed.set_footer(
        text=footer_text, icon_url=user.display_avatar.url if user.display_avatar else None
    )

    return embed, files


def create_reading_embed(reading: Reading, user: discord.User, spread: Spread) -> discord.Embed:
    """
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"✨ {spread.num_cards} cards drawn • Take a moment to reflect..."
        ),
        color=COLORS["primary"],
        timestamp=reading.timestamp,
    )
    ...
    for i, result in enumerate(reading.results):
        ...
        embed.add_field(name=f"{position_title}\n{card_subtitle}", value=field_value, inline=False)
    ...
    footer_text = (
        f"Reading for {user.display_name} • {spread.num_cards} cards • "
...
def _create_reading_summary(reading: Reading, spread: Spread) -> str:
    ...
    if major_count >= len(reading.results) / 2:
        ...
        suit_messages = {
            ...
            "PENTACLES": (
                "🌍 **Earth energy** — Material concerns and practical matters are the focus"
            ),
        }
...
def create_spread_info_embed() -> discord.Embed:
    ...
    from .data import SPREAD_DISPLAY_ORDER, SPREADS
    ...
    for spread_key in SPREAD_DISPLAY_ORDER:
        ...
        if spread.difficulty == "Beginner":
            emoji = "⭐"
        elif spread.difficulty == "Intermediate":
            emoji = "🌙"
        else:
            emoji = "✨"

        if len(spread.description) > 60:
            value = f"*{spread.description[:60]}...*\n"
        else:
            value = f"*{spread.description}*\n"
        value += (
            f"📊 {spread.num_cards} cards • ⏱️ ~{spread.estimated_time} • 📈 {spread.difficulty}"
        )

        embed.add_field(name=f"{emoji} {spread.name}", value=value, inline=True)
...
def create_journal_embed(
    ...
    for i, entry in enumerate(page_readings, start + 1):
        ...
        if isinstance(entry_data, dict):
            date_str = entry_data.get("timestamp", "Unknown date")[:10]
            spread = entry_data.get("spread_type", "Unknown spread")
            question = entry_data.get("question", "")
            cards = entry_data.get("cards", [])
        else:
            date_str = str(entry_data.timestamp)[:10]
            spread = entry_data.spread_type
            question = entry_data.question
            cards = entry_data.cards
        ...
        embed.add_field(name=f"#{i} • {date_str}", value=value, inline=False)
...
def create_theme_selection_embed(
    themes: List, user_name: str, is_first_time: bool = False
) -> discord.Embed:
    ...
    for i, theme in enumerate(themes[:10], 1):
        ...
        embed.add_field(name=f"{emoji} {i}. {theme.name}", value=f"*{desc}*", inline=True)
...
def create_theme_selected_embed(theme, user_name: str) -> discord.Embed:
    ...
    embed.add_field(name="About This Deck", value=theme.description, inline=False)
