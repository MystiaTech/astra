...
from .journal import get_journal_manager, JournalManager
from .reading import Reading
from .embeds import (
    create_journal_embed,
    create_journal_entry_embed,
...
class JournalCommands(commands.Cog):
    ...
    async def tarot_journal_delete(
        self,
        interaction: discord.Interaction,
        entry_id: int
    ):
        ...
        if success:
            await interaction.followup.send(
                f"🗑️ Entry #{entry_id} has been deleted from your journal.",
                ephemeral=True
            )
        else:
            await interaction.followup.send(
                "❌ Could not delete entry. It may not exist.",
                ephemeral=True
            )
    ...
    async def tarot_journal_stats(self, interaction: discord.Interaction):
        ...
        if stats["total_readings"] == 0:
            await interaction.followup.send(
                "📊 You haven't saved any readings yet. Start with `/tarot-single`!",
                ephemeral=True
            )
            return
        ...
        embed.add_field(name="Favorite Spread", value=stats["most_common_spread"] or "N/A", inline=True)
