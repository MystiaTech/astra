...
class ThemeCommands(commands.Cog):
    ...
    async def tarot_themes(self, interaction: discord.Interaction):
        ...
        if not themes:
            await interaction.response.send_message(
                "No themes available. Please contact the server administrator.", ephemeral=True
            )
            return
    ...
    async def tarot_theme(self, interaction: discord.Interaction):
        ...
        if len(themes) == 0:
            await interaction.response.send_message("No themes available.", ephemeral=True)
            return

        if len(themes) == 1:
            await interaction.response.send_message(
                f"Only one theme is available: **{themes[0].name}**", ephemeral=True
            )
            return
        ...
        embed = create_theme_selection_embed(
            themes=themes, user_name=interaction.user.display_name, is_first_time=is_first_time
        )
        ...
        select = discord.ui.Select(
            placeholder="Choose your deck theme...", options=options, custom_id="theme_select"
        )
        ...
        async def select_callback(interaction: discord.Interaction):
            ...
            if theme:
                ...
                confirm_embed = create_theme_selected_embed(
                    theme=theme, user_name=interaction.user.display_name
                )
    ...
    async def tarot_theme_preview(self, interaction: discord.Interaction, theme_name: str):
        ...
        if not theme:
            ...
            await interaction.response.send_message(
                f"Theme '{theme_name}' not found. Available: {available}", ephemeral=True
            )
            return
...
async def prompt_theme_selection(bot, interaction: discord.Interaction) -> bool:
    ...
    embed = create_theme_selection_embed(
        themes=themes, user_name=interaction.user.display_name, is_first_time=True
    )
    ...
    for theme in themes[:25]:
        ...
        options.append(
            discord.SelectOption(
                label=theme.name, value=theme.id, description=theme.description[:100], emoji=emoji
            )
        )

    select = discord.ui.Select(placeholder="Choose your deck theme...", options=options)
