...
class TestSpreadRegistry:
    ...
    def test_spreads_have_required_attributes(self, sample_spreads):
        ...
        for key, spread in sample_spreads.items():
            ...
            assert len(spread.positions) == spread.num_cards, f"{key} positions count mismatch"
            assert spread.best_for, f"{key} should list best uses"
            assert spread.difficulty in [
                "Beginner",
                "Intermediate",
                "Advanced",
...
class TestCelticCross:
    ...
    def test_celtic_has_traditional_positions(self, sample_spreads):
        ...
        assert any(
            "Present" in n or "Situation" in n for n in names
        ), "Should have present situation"
        assert any("Challenge" in n or "Cross" in n for n in names), "Should have challenge/cross"
        assert any("Foundation" in n or "Root" in n for n in names), "Should have foundation"
        assert any("Future" in n or "Outcome" in n for n in names), "Should have outcome"
...
class TestRelationshipSpread:
    ...
    def test_relationship_has_self_and_other(self, sample_spreads):
        ...
        assert any("you" in n or "self" in n for n in names), "Should have self position"
        assert any(
            "other" in n or "partner" in n for n in names
...
class TestCareerSpread:
    ...
    def test_career_has_professional_focus(self, sample_spreads):
        ...
        assert any(
            term in combined for term in career_terms
...
class TestSpreadDocumentation:
    ...
    def test_all_positions_have_descriptions(self, sample_spreads):
        ...
        for key, spread in sample_spreads.items():
            for i, pos in enumerate(spread.positions):
                ...
                assert len(pos.description) > 20, f"{key} position {i+1} description too short"
    ...
    def test_spreads_list_best_uses(self, sample_spreads):
        ...
        for key, spread in sample_spreads.items():
            assert len(spread.best_for) >= 3, f"{key} should list at least 3 best uses"
