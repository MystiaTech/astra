...
class TestSessionManagement:
    ...
    async def test_cannot_acquire_when_session_active(self, bot):
        ...
        bot.active_session = ReadingSession(
            user_id=99999, username="OtherUser", spread_type="celtic", started_at=datetime.now()
        )
    ...
    async def test_session_release_allows_new_session(self, bot):
        ...
        bot.active_session = ReadingSession(
            user_id=12345, username="TestUser", spread_type="single", started_at=datetime.now()
        )
...
class TestCardDrawing:
    ...
    def test_draws_correct_number_of_cards(self):
        ...
        for spread_type, expected_count in spread_card_counts.items():
            reading = cog._draw_cards(
                spread_type=spread_type, question="Test question", allow_reversed=False
            )

            assert (
                len(reading.results) == expected_count
    ...
    def test_no_duplicate_cards_in_reading(self):
        ...
        reading = cog._draw_cards(spread_type="celtic", question="Test", allow_reversed=False)

        card_ids = [(r.card.name, r.card.suit) for r in reading.results]
        unique_cards = set(card_ids)

        assert len(card_ids) == len(unique_cards), "Should not have duplicate cards in reading"
    ...
    def test_reversed_cards_when_allowed(self):
        ...
        for _ in range(20):
            reading = cog._draw_cards(spread_type="three", question="Test", allow_reversed=True)
    ...
    def test_no_reversed_cards_when_disallowed(self):
        ...
        reading = cog._draw_cards(spread_type="three", question="Test", allow_reversed=False)

        assert all(
            not r.reversed for r in reading.results
...
class TestReadingModel:
    ...
    def test_reading_has_timestamp(self):
        ...
        reading = Reading(spread_type="single", question="Test", results=[])
    ...
    def test_count_reversed(self):
        ...
        reading = Reading(spread_type="three", question="Test", results=results)
