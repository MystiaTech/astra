...
class TestCardStructure:
    ...
    def test_all_cards_have_image_placeholder(self, sample_deck):
        ...
        for card in sample_deck:
            ...
            assert card.image_placeholder.endswith('.png'), \
    ...
    def test_minor_arcana_have_elements(self, minor_arcana_cards):
        ...
        for card in minor_arcana_cards:
            if card.number <= 10:  # Number cards
                ...
                assert card.element == expected, \
                    f"{card.name} should have element {expected}"
...
class TestCardMeaningsAccuracy:
    ...
    def test_death_card_not_literal(self, sample_deck):
        ...
        assert "physical death" not in death.meaning.lower() or \
               "literal death" not in death.meaning.lower(), \
            "Death card should not focus on literal death"
        assert "transformation" in death.meaning.lower() or \
               "change" in death.meaning.lower() or \
               "ending" in death.meaning.lower(), \
    ...
    def test_devil_card_addresses_bondage(self, sample_deck):
        ...
        assert any(word in devil.meaning.lower() for word in 
                  ["attachment", "addiction", "restriction", "shadow", "limitation"]), \
    ...
    def test_fool_card_addresses_beginnings(self, sample_deck):
        ...
        assert any(word in fool.meaning.lower() for word in
                  ["beginning", "new", "potential", "innocence", "faith"]), \
    ...
    def test_suit_meanings_are_consistent(self, minor_arcana_cards):
        ...
        assert any(word in wands.meaning.lower() for word in 
                  ["inspiration", "creativity", "passion", "action"]), \
        ...
        assert any(word in cups.meaning.lower() for word in
                  ["emotion", "love", "feeling", "relationship"]), \
...
class TestCardDisplay:
    ...
    def test_major_arcana_display_name(self, major_arcana_cards):
        ...
        for card in major_arcana_cards:
            ...
            assert "of" not in name, f"Major Arcana {card.name} should not include 'of'"
    ...
    def test_minor_arcana_display_name(self, minor_arcana_cards):
        ...
        for card in minor_arcana_cards:
            ...
            assert card.suit.name.title() in name, \
                f"{card.name} should include suit name"
    ...
    def test_keywords_are_appropriate(self, sample_deck):
        ...
        for card in sample_deck:
            for keyword in card.keywords:
                ...
                assert keyword.isalpha() or " " in keyword, \
